...
folder are mapped to the application's root namespace, 'CultivateCode'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "CultivateCode"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'CultivateCode' in the same way
as `"CultivateCode/sass/src"`.
